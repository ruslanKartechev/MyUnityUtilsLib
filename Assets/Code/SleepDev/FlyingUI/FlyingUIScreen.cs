    ...
    public class FlyingUIScreen : MonoBehaviour
    {
        ...
        public void FlyFromTo(Sprite icon, Vector3 fromPos, Vector3 endPos, int count, float time = 0f)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                it.FlyFromTo(randFromPos, endPos, time, null, _defaultDelay);
            }
        }

        public void FlyFromTo(Sprite icon, Vector3 fromPos, Vector3 endPos, Action onFlyEnd, float time = 0f)
        {
            if (time <= 0f)
                time = _defaultFlyTime;
            var it = (FlyingElement)_elementsPool.GetOne();
            it.Pool = _elementsPool;
            it.SetIcon(icon);
            it.FlyFromTo(fromPos, endPos, time, onFlyEnd, _defaultDelay);
        }
