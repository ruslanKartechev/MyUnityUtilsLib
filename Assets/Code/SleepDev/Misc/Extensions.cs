...
namespace SleepDev
{
    ...
    {
        public static Transform FindClosestTransform(this List<Transform> transforms, Vector3 origin)
        {
            Transform targetPoint = null;
            var minDist = float.MaxValue;
            foreach (var tempPoint in transforms)
            {
                var dist = (tempPoint.position - origin).sqrMagnitude;
                if (dist < minDist)
                {
                    minDist = dist;
        ...
        public static float MakeSignedAngle(this float angle360)
        {
            if (angle360 >= 180)
                angle360 -= 360;
            return angle360;
