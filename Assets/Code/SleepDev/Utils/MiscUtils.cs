...
#region  Formatting
        ...
        public static Transform FindClosestPoint(this List<Transform> points, Vector3 center)
        {
            var result = points[0];
...
#region Transforms and Hierarchy
...
#region Array Utils
        ...
        public static async Task WaitGameTime(float timeSec, CancellationToken token)
        {
            while (!token.IsCancellationRequested && timeSec > 0)
            {
        ...
        public static async Task WaitGameTime(int timeMs, CancellationToken token)
        {
            while (!token.IsCancellationRequested && timeMs > 0)
            {
                timeMs -= (int)(Time.deltaTime * 1000);
                await Task.Yield();
            }
        }
    }
